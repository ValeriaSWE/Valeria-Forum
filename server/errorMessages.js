export const SomethingWrong = "Ajdå! Något gick fel"

export const UsernameTaken = "Användarnamet är redan uptaget."

export const UserDoesntExists = "Användaren finns inte."

export const PasswordDontMatch = "Lösenorden matchar inte."

export const IncorrectPassword = "Ogiltligt användarnamn eller lösenord."

export const InvalidToken = "Autentiseringsfel. Försök igen."

export const AuthLevel = "Inte tillräckliga roller."